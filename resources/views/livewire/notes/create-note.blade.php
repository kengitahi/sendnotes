...
new class extends Component {
    ...
    public function submit()
    {
        ...
        auth()
            ->user()
            ->notes()
            ->create(['title' => $this->noteTitle, 'body' => $this->noteBody, 'recipient' => $this->noteRecipient, 'send_date' => $this->noteSendDate, 'is_published' => false]);
...
<div>
    <form class="space-y-4" wire:submit='submit'>
        <x-input label='Title of the note' placeholder="What's the note's title?" right-icon="inbox"
        ...
        <x-input label='Recipient of the note'
            placeholder="Who are you sending the note to? (Please enter an email address)" right-icon="at-symbol"
            type="email" wire:model='noteRecipient' />
        <x-datetime-picker :min="now()" label="Send Date" placeholder="When would you like to send the note?"
